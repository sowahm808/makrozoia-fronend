    ...
    @if (profile$ | async; as profile) {
      ...
      <div class="cards">
        <article>
          ...
          <p><strong>Location:</strong> {{ profile.headquartersLocation }}</p>
        </article>
        <article>
          <h3>Recommended next steps</h3>
          <ol>
            <li>Review your selected services with a Makrozoia architect.</li>
            <li>Schedule a discovery workshop.</li>
            <li>Confirm project scope, timeline, and delivery model.</li>
          </ol>
        </article>
        <article>
          <h3>Actions</h3>
          <a class="button" routerLink="/company-profile"
            >Update company profile</a
          >
        </article>
      </div>
      <h2>Services selected</h2>
      <div class="pill-grid">
        @for (service of profile.servicesInterestedIn; track service) {
          <span>{{ service }}</span>
        }
      </div>
    } @else {
